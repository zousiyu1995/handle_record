...
def main() -> None:
    ...
    with open("summary.json", "r", encoding="utf-8") as json_file:
        ...
        opening_idiom = [
            i.opening_idiom for i in records if i.opening_idiom != None
        ]
    ...
    print(f"总用时：{print_time(sum(time, timedelta(0, 0)).seconds)}")
    print(f"平均用时：{print_time(mean(time).seconds)}")
    print(f"最长用时：{print_time(max(time).seconds)}")
    print(f"最短用时：{print_time(min(time).seconds)}")
